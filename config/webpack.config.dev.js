...
const getStyleLoaders = (cssOptions, options, loader) => {
  const loaders = [
...
// this function generates all necessary CSS loaders from user's configuration
const getCssLoaders = (config) => [
  // this loader handles any CSS placed in "node_modules"
  {
    test: cssRegex,
  ...
  // these loaders are loaded from user's configuration, but all of them can be applied only
  // to "src" directory
  ...config.reduce((acc, {
    loader,
    options,
    pattern: {global, module},
  }) => {
    if (global) {
      acc.push({
        test: global,
